...
# 5. Intala el Ingress Controller
...
# 5-A. Etiqueta nodos para que el Ingress Controller pueda programarse en ambos
...
# 5-B. Espera a que el controller esté OK
...
# 6-A. Esperar a que todos los pods estén listos y ejecutar el script de inicialización de CrateDB
...
# 6-B. Reiniciar Grafana para que coja el nuevo configMap

kubectl rollout restart deployment grafana -n tfg-iot

# 6-C. Esperar a que Metrics-server esté listo y finaliza la configuración
